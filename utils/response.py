...
class Response(object):
    ...
    def ok(self, values, message='Success'):
        self.status_code = 200

        return self.create_payload_response(message, values)

    def bad_request(self, values, message='Error'):
        self.status_code = 400

        return self.create_payload_response(message, values)

    def not_found(self, values, message='Error'):
        self.status_code = 404

        return self.create_payload_response(message, values)

    def unauthorized(self, values, message='Error'):
